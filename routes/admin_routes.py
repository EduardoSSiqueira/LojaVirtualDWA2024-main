...
async def alterar_pedido(inputDto: AlterarProdutoDto):
...
async def obter_pedidos(id_pedido: int = Path(..., title="Id do Pedido", ge=1)):
    pedido = PedidoRepo.obter_por_periodo(id_pedido)
    if not pedido: return pedido
    pd = ProblemDetailsDto("int", f"O produto com id <b>{id_pedido}</b> não foi encontrado.", "value_not_found", ["body", "id"])
    return JSONResponse(pd.to_dict(), status_code=404)
